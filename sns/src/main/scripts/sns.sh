...
APP_MAIN=com.tt.sns.SnsApplication

APP_PID_FILE=$APP_HOME/tmp/sns.pid
...
startup()
{
    ...
    if [ ! -f $APP_PID_FILE ];then
        nohup java $JAVA_OPTS -cp $APP_HOME/conf/:$APP_HOME/lib/*:. $APP_MAIN 2>>$APP_HOME/logs/sns.out 1>>$APP_HOME/logs/sns.out &
        echo $!>$APP_PID_FILE
        #判断服务是否正常启动成功
        ...
        if [ `ps -ef |grep $APP_PID |grep -v "grep"|wc -l` -eq 0 ]; then
            ...
            echo 'sns server start up failure.'
        else
             echo 'sns server start up successfully.'
        fi
    else
        echo 'sns server is already running.'
    fi
}
...
shutdown()
{
    if [ -f $APP_PID_FILE ];then
        ...
        echo '.'
        rm $APP_PID_FILE
        echo 'sns service shutdown successfully.'
    else
        echo 'sns service is not running.'
    fi
}
